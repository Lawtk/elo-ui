<template>
    <el-card>
        ...
        <div style=" display: flex;flex-direction: row;">
            <el-card v-for="item in list" shadow="hover" style="width: 280px;margin-right: 20px;">
                <template #header>
                    <span>{{ item.name }} </span>
                </template>
                <p>用户名:{{ item.username }}</p>
                <p>密码:{{ item.password }}</p>
                <p>数据库类型:<el-tag size="small">{{ item.databaseType }}</el-tag></p>
                <template #footer>
                ...
                <el-col :span="6">
                    <el-form-item label="类型">
                        <el-select v-model="dataSource.databaseType">
                            <el-option v-for="item in optionsMap.get('database_type')" :key="item.id"
...
export default {
    ...
    methods: {
        ...
        getList() {
            commQuery(this.model_url + "/queryList", this.queryParams, this.pageParams).then(res => {
                if (res.success) {
